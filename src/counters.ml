...
let update { counters } msg =
  ...
  | `Inc pos            -> { counters = IntMap.add pos (IntMap.find pos counters + 1) counters}
  | `Dec pos            -> { counters = IntMap.add pos (IntMap.find pos counters - 1 |> ensure_pos) counters}
